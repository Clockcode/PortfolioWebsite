jQuery(document).ready(function () {
  ...
  //show the gmail mailing screen if user clicks the show-email button
  intro.on("click", 'a[data-action="show-email"]', function (event) {
    event.preventDefault();
    window.open(
      "https://mail.google.com/mail/?view=cm&fs=1&to=cagdasmuldur@gmail.com",
      "_blank"
    );
  });

  //show the resume if user clicks the show-resume button
  ...
  intro.on("click", function (event) {
    ...
    if (
      intro.hasClass("projects-visible") &&
      !$(event.target).is('a[data-action="show-projects"]')
    ) {
  ...
  projectsContainer.on("click", ".cd-slider a", function (event) {
    ...
    if ($(this).parent("li").next("li").is(".current") && mq == "desktop") {
      ...
      $(this).parent("li").prev("li").prev("li").prev("li").is(".current") &&
      mq == "desktop"
    ) {
  ...
  $(document).keyup(function (event) {
    ...
    if (
      event.which == "37" &&
      intro.hasClass("projects-visible") &&
      !sliderNav.find(".prev").hasClass("inactive") &&
      mq == "desktop"
    ) {
      prevSides(projectsSlider);
    } else if (
      event.which == "39" &&
      intro.hasClass("projects-visible") &&
      !sliderNav.find(".next").hasClass("inactive") &&
      mq == "desktop"
    ) {
      nextSides(projectsSlider);
    } else if (
      event.which == "27" &&
      singleProjectContent.hasClass("is-visible")
    ) {
      singleProjectContent.removeClass("is-visible");
    }
  });
  ...
  projectsSlider.on("swipeleft", function () {
    var mq = checkMQ();
    if (!sliderNav.find(".next").hasClass("inactive") && mq == "desktop")
      nextSides(projectsSlider);
  });

  projectsSlider.on("swiperight", function () {
    var mq = checkMQ();
    if (!sliderNav.find(".prev").hasClass("inactive") && mq == "desktop")
      prevSides(projectsSlider);
  });
  ...
  function setSliderContainer() {
    ...
    if (mq == "desktop") {
      ...
      slideCurrentIndex != 0 &&
        setTranslateValue(
          projectsSlider,
          slideCurrentIndex * (slideWidth + marginLeft) + "px"
        );
    } else {
  ...
  function nextSides(slider) {
    var actual = slider.children(".current"),
      ...
      width = actual.width(),
      marginLeft = Number(
        slider.children("li").eq(1).css("margin-left").replace("px", "")
      );
